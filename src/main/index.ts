...
function init() {
    ...
    app.whenReady().then(async () => {
        if (process.platform === "win32") app.setAppUserModelId("dev.papertobi.skibiditop");
...
if (!app.requestSingleInstanceLock({ IS_DEV })) {
    if (IS_DEV) {
        console.log("Skibiditop is already running. Quitting previous instance...");
        init();
    } else {
        console.log("Skibiditop is already running. Quitting...");
        app.quit();
    }
} else {
    init();
